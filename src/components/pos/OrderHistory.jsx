...
export default function OrderHistory({ isOpen, onClose, orders, onMarkAsPaid, onEditOrder }) {
      ...
      <div className="relative w-full sm:max-w-md h-full bg-surface shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-right duration-300">
        <div className="p-4 sm:p-6 border-b border-border flex justify-between items-center bg-secondary/20 shrink-0">
          <div>
            ...
            <h2 className="text-xl font-black uppercase text-accent leading-none mt-1">Lịch sử đơn hàng</h2>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 custom-scrollbar">
          {orders.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center opacity-20 py-20">
              ...
              <p className="text-sm font-black uppercase tracking-widest">Chưa có đơn nào</p>
            </div>
          ) : (
            [...orders].reverse().map(order => (
              <div key={order.id} className="bg-background rounded-[1.5rem] p-4 border border-border shadow-sm">
                <div className="flex items-center justify-between border-b border-border/50 pb-3 mb-3">
                  <div>
                    <div className="flex items-center gap-2">
                       <p className="font-black text-sm uppercase text-accent tracking-tight">#{order.id}</p>
                       {order.status === 'pending' && (
                         <button title="Sửa đơn này" onClick={() => onEditOrder(order.id)} className="text-primary hover:bg-primary/10 p-1 rounded-md transition-colors">
                           <Edit3 size={14} />
                         </button>
                       )}
                    </div>
                    <p className="text-[10px] font-bold text-accent/50 mt-0.5">
                      {new Date(order.timestamp).toLocaleTimeString('vi-VN')} - {new Date(order.timestamp).toLocaleDateString('vi-VN')}
                    </p>
                  </div>
                    ...
                    <div key={idx} className="flex justify-between items-start">
                      <div className="flex gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="font-bold text-xs text-accent truncate">{item.name}</p>
                          {(item.tags?.length > 0 || item.customNote) && (
                            <p className="text-[10px] text-accent/60 italic truncate mt-0.5">
                              {item.tags?.map(t => typeof t === 'string' ? t : t.label).join(', ')} 
